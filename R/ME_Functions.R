...
stdizeCVCovarNames <- function(IDColsME,StrainME,EnvVarME,varEnv_MECV,fix_MECV,factVar_MECV){

	IDColsME[which(IDColsME %in% StrainME)] <- "Strain"

	if(!is.null(EnvVarME)){ ## FIX bug8: wrap both env conditionals in null-guard

		if(varEnv_MECV == EnvVarME){
			IDColsME[which(IDColsME %in% varEnv_MECV)] <- "Env"
			varEnv_MECV_Mod <- "Env"
		}else{ varEnv_MECV_Mod <- varEnv_MECV}

		if(fix_MECV == EnvVarME){
			IDColsME[which(IDColsME %in% EnvVarME)] <- "Env" ## FIX bug7: use EnvVarME as search key (fix_MECV==EnvVarME here, and prior block may have renamed it)
			fix_MECV_Mod <- "Env"
		}else{fix_MECV_Mod <- fix_MECV}

		if(factVar_MECV == EnvVarME){
			IDColsME[which(IDColsME %in% EnvVarME)] <- "Env" ## FIX bug7: use EnvVarME as search key (fix_MECV==EnvVarME here, and prior block may have renamed it)
			factVar_MECV_Mod <- "Env"
		}else{factVar_MECV_Mod <- factVar_MECV}

	} ## end if(!is.null(EnvVarME)) FIX bug8

	return(list(varEnv_MECV_Mod,fix_MECV_Mod,factVar_MECV_Mod,IDColsME)) ## FIX bug6: return IDColsME as third element
...
###
### summarize_ME_CV_fits_V0
### Legacy summarize function (original version with DT + IDColsList args)
###

summarize_ME_CV_fits_V0 <- function(
  fit_obj,          # output from fitMEModels_CV_fits()
...
###
### fitMEModels_KFoldCV_V5
### Legacy k-fold CV function (V5)
###
...
###
### fit_DTfold_V0
### Legacy fold-fitting helper (original EnvRtype-based approach)
###

fit_DTfold_V0 <- function(DT_fold, KG, KE, method, fitEnvModels, genoDat, trait) {
    Y   <- DT_fold[, c("env","gid","value")]
    y   <- "value"; gid <- "gid"; env <- "env"
    ne  <- length(unique(Y$env))
    ...
    if (!fitEnvModels && is.null(KE) && !is.null(KG) && is.null(method)) {
      if (ne > 1) {
        ...
        fitModels$MM  <- list(yHat = fit_MM$yHat,   VarComp = fit_MM$VarComp)
        fitModels$MDs <- list(yHat = fit_MDs$yHat,  VarComp = fit_MDs$VarComp)
        fitModels$MDe <- list(yHat = fit_MDe$yHat,  VarComp = VC_MDe)
        ...
        fitModels$SM <- list(yHat = fit_SM$yHat, VarComp = fit_SM$VarComp)
        if (store_full_fits) raw_store <- list(SM = fit_SM)
      }
      ...
      fitModels$MM  <- list(yHat = fit_EMM$yHat,   VarComp = fit_EMM$VarComp)
      fitModels$MDs <- list(yHat = fit_EMDs$yHat,  VarComp = fit_EMDs$VarComp)
      fitModels$MDe <- list(yHat = fit_EMDe$yHat,  VarComp = VC_EMDe)
	...
	if (ne > 1) {
	  ...
	  EMM  <- get_geno_kernel(Y,X,KG,KE,intercept.random=FALSE,model="MM",method="GK",dimension_KE=NULL)
      EMDs <- get_geno_kernel(Y,X,KG,KE,intercept.random=FALSE,model="MDs",method="GK",dimension_KE=NULL)
      EMDe <- get_geno_kernel(Y,X,KG,KE,intercept.random=FALSE,model="MDe",method="GK",dimension_KE=NULL)
