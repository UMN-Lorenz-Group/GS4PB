...
app_server <- function(input, output, session) {
  ...
  #   TargtIDCol <- reactive(input$TargetIDCol)
  #   TargetIDs(as.character(TargetTab()[,TargtIDCol()]))
  # })
    ...
    if(impMethod()=="AlphaPlantImpute"){
      ...
      output$APIUI <- renderUI({
        ...
        fluidPage(
          ...
          fluidRow(
            column(
              width = 10,
              tags$strong("Step 1: Building the haplotype library"),
              tags$ul(
          ...
          fluidRow(
            column(
              width = 10,
              tags$strong("Step 2: Genotype file imputation"),
              tags$ul(
          ...
          fluidRow(
            ...
            column(width=5,tags$strong(tags$h3("Build Haplotype Library "))),
            column(width=1),column(width=5,tags$strong(tags$h3("Impute Genotype Table ")))
          ),
     ...
     "))),
          ...
          fluidRow(
            ...
            column(width = 4),  # Adjust the width or remove if not needed
            column(width = 5,
  ...
  observeEvent(input$build_library,{
    ...
    #       lines <- readLines(temp_file(), warn = FALSE)
    #       shinyjs::enable("impute_APIdata")
    #       rProcess$kill()  # Ensure the background process is terminated
    #       mssgAPIBuildOut(paste(lines, collapse = "\n", "Build Completed",sep="\n"))
    #       return(paste(lines, collapse = "\n", "Build Completed",sep="\n"))
    #     } else {
    #       return("Process completed, but no output file found.")
    #     }
    #   }
    #
    #   if (!processAlive()) {
    ...
    # output$message <- renderText({
    #   invalidateLater(1000, session) # Re-run this reactive every second
    #
    #   if (exists("rProcess")) {
    #     print("rProcess exists")
    #     processAlive <- reactive({
    #       invalidateLater(1000, session) # Check every second
    #       rProcess$is_alive()
    #     })
    ...
    #       if (file.exists(temp_file())) {
    #         lines <- readLines(temp_file(), warn = FALSE)
    #         shinyjs::enable("impute_APIdata") # Enable UI element
    #         rProcess$kill() # Ensure the background process is terminated
    #         message <- paste(lines, "Build Completed", sep = "\n")
    #         mssgAPIBuildOut(message) # Store the message
    #         return(message)
    #       } else {
    #         rProcess$kill() # Ensure the process is terminated
    #         return("Process completed, but no output file found.")
    #       }
    #     }
    #   }else {
    #     print("rProcess does not exist")
    #   }
    #
    #   # Additional condition when `rProcess` does not exist
    #
    #   # Default message if no conditions are met
    #   return("No process running or output available.")
    # })
    #
    # })
  ...
  DT_Filt_List <- reactive({
    print("prIn2")
    if(isPhenoME() && inherits(processedData(),"list")){
      processedData()[[1]]
    }
  })
    ...
    if(checkSEData()==TRUE){
      ...
      b <- callr::r_bg(function(getOptimalTS,getRandomTS,getTSComparisons,getTSComparisonsMT,predictionData,Trait,nTraits,noCandidates,nTrainToSelect,GAParameters,nSelTraits,TargetIDs,temp_file4b_path){
        ...
        if (!is.null(Train_STPGA) && !is.null(Train_Random) &&
          ...
          return(list(TSOptOutputList,Train_STPGA,Train_Random))
          cat("\n")
        }else{return(NULL)}
  ...
  Train_STPGA <- reactiveVal(NULL)
  Train_Random <- reactiveVal(NULL)
  ...
  processStatus4b <- reactive({
    ...
    if (!is.null(rProcess4b()) && rProcess4b()$is_alive()) {
      ...
      if (file.exists(temp_file4b())) {
        ...
        TSOptOutputTab(rProcess4b()$get_result()[[1]])
        Train_STPGA(rProcess4b()$get_result()[[2]])
        Train_Random(rProcess4b()$get_result()[[3]])
        ...
        if(nSelTraits >1){
          ...
          for(nSelT in 1:nSelTraits){
            ...
            if (is.null(trOutputDFComb)) {
              ...
              if(inherits(outDF_List[[2]],"data.frame")){
  ...
  output$Ranked_Lines_for_SelectionST <- renderTable({
    if(STGPOut()=="Target"){
      if(inherits(gpOutputListST()[[2]],"data.frame")){
        outputListST(gpOutputListST()[[2]])
        return(gpOutputListST()[[2]][1:10,])

      }else if(inherits(gpOutputListST()[[2]],"character")){
        gpOutputListST()[[2]]
      }
  ...
  output$Ranked_Lines_for_SelectionMT <- renderTable({

    if(nSelTraits()>1){

      if(MTGPOut()=="Target"){
        if(inherits(gpOutputListMT()[[2]],"data.frame")){
          outputListMT(gpOutputListMT()[[2]])
          return(gpOutputListMT()[[2]][1:20,])
        }else if(inherits(gpOutputListMT()[[2]],"character")){
          gpOutputListMT()[[2]]
        }
  ...
  observe({
    ...
    output$plots <- renderPlotly({  # Use renderPlotly for plotly plots
      if(nSelTraits() == 1 && STGPOut() == "Target"){
  ...
  observe({
    ...
    if(nSelTraits()>1 && STGPOut()=="Target" && inherits(gpOutputListST()[[2]],"data.frame")){
